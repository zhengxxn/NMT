...
class TransformerWithFullAdapter(nn.Module):
    ...
    def __init__(self,
        ...
        for domain in domain_dict.keys():
            self.src_embedding_layer.adapter_layers[domain].w_1.weight = self.trg_embedding_layer.adapter_layers[domain].w_1.weight
            self.src_embedding_layer.adapter_layers[domain].w_1.bias = self.trg_embedding_layer.adapter_layers[domain].w_1.bias
            self.src_embedding_layer.adapter_layers[domain].w_2.weight = self.trg_embedding_layer.adapter_layers[domain].w_2.weight
            self.src_embedding_layer.adapter_layers[domain].w_2.bias = self.trg_embedding_layer.adapter_layers[domain].w_2.bias

            self.generator.adapter_layers[domain].w_1.weight = self.trg_embedding_layer.adapter_layers[domain].w_1.weight
            self.generator.adapter_layers[domain].w_1.bias = self.trg_embedding_layer.adapter_layers[domain].w_1.bias
            self.generator.adapter_layers[domain].w_2.weight = self.trg_embedding_layer.adapter_layers[domain].w_2.weight
            self.generator.adapter_layers[domain].w_2.bias = self.trg_embedding_layer.adapter_layers[domain].w_2.bias
    ...
    def forward(self, src, src_mask, trg_input, trg, trg_mask, target_domain):
        ...
        log_probs = self.generator(decoder_logits, target_domain)
        # loss = self.criterion(
    ...
    def decode_step(self, input, decode_state):
        ...
        logits = logits[:, -1, :]
        log_prob = self.generator(logits, self.target_domain)
    ...
    def encode(self, src, src_mask, target_domain):
        self.target_domain = target_domain
        input_embedding = self.src_embedding_layer(src, target_domain)
    ...
    def decode(self, trg_input, trg_mask, decoder_state, target_domain, test=False):
        trg_input_embedding = self.trg_embedding_layer(trg_input, target_domain)
