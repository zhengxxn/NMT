...
class TransformerWithMultiAdapter(nn.Module):
    ...
    def forward(self, src, src_mask, trg_input, trg, trg_mask, used_domain_list):

        decoder_state = self.prepare_for_decode(src, src_mask, used_domain_list, require_adapter_output=True)

        decoder_logits, adapter_output, _, _ = self.decode(trg_input, trg_mask, decoder_state, used_domain_list)
    ...
    def prepare_for_decode(self, src, src_mask, used_domain_list, require_adapter_output=False):
        # src = batch.src
        # src_mask = batch.src_mask

        assert src_mask is not None
        self.used_domain_list = used_domain_list
        encoder_state = self.encode(src, src_mask, used_domain_list)
        decoder_state = {
    ...
    def encode(self, src, src_mask, used_domain_list):
        input_embedding = self.src_embedding_layer(src)
        encoder_state = self.encoder(input_embedding, src_mask, used_domain_list=used_domain_list)
        return encoder_state

    def decode(self, trg_input, trg_mask, decoder_state, used_domain_list, test=False):
