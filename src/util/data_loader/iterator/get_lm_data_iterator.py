...
from util.data_loader.iterator.token_level_bucket_iterator import TokenLevelBucketIterator, text_max_token_batch_size_fn
...
def get_train_iterator(dataset, batch_size, device, token_level=True):
    if token_level:
        it = TokenLevelBucketIterator(dataset=dataset,
                                      ...
                                      batch_size_fn=text_max_token_batch_size_fn,
                                      train=True,
                                      shuffle=True)
    else:
        it = BucketIterator(
