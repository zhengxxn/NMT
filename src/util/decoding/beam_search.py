...
class BeamSearch:
    ...
    def search(
        ...
        for timestep in range(self.max_steps - 1):
            ...
            expanded_last_length = (
                last_length.unsqueeze(2)
                .expand(batch_size, self.beam_size, self.per_node_beam_size)
                .reshape(batch_size * self.beam_size, self.per_node_beam_size)
            )
        ...
        if self.length_penalty:
            ...
            valid_length = torch.where(valid_length == 0,
                                       fill_value,
                                       valid_length)  # confirm length is not 0
            valid_length = valid_length.float()
            if self.alpha > 0:
                length_penalty = ((5.0 + valid_length) ** self.alpha) / ((5.0 + 1.0) ** self.alpha)
        ...
        sorted_last_log_probabilities, sorted_index = torch.topk(last_log_probabilities, k=self.beam_size, dim=-1)
