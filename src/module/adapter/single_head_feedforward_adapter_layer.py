...
class SingleHeadFeedForwardAdapterLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        query = x
        # b[batch size, seq len, input dim]

        key = global_key_vector
        # [mem count, input_dim]

        # score = torch.matmul(query, key.transpose(-1, -2))  # [batch, seq len, mem count]
        ...
        value = global_value_vector.transpose(0, 1)
        # [mem count, input dim]

        # multi_head_output = torch.matmul(score, value)  # [batch size, head num, seq len, head_dim]
        multi_head_output = torch.einsum('ij,abi->abj', (value, score))
        # a: batch size, b: head num, c: seq len, i: mem count, j: head dim
